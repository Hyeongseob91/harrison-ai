...
if clear_btn:
    ...
    if "chatbot_session_id" in st.session_state:
        ...
        if session_id in st.session_state["chatbot_store"]:
            st.session_state["chatbot_store"][session_id].clear()
...
# 초기화 : 세션 ID 정의 및 저장
if "chatbot_store" not in st.session_state:
    st.session_state["chatbot_store"] = {}
...
def add_message(role, message):
    st.session_state["chatbot_messages"].append(ChatMessage(role=role, content=message))

# 기능-3 (세션 아이디 반환)
def get_session_history(session_id):
    if session_id not in st.session_state["chatbot_store"]:
        st.session_state["chatbot_store"][session_id] = ChatMessageHistory()

    return st.session_state["chatbot_store"][session_id]
